    ...
    <footer className="w-full flex flex-col">
      ...
      <div className="w-full flex flex-col md:flex-row min-h-[500px]">
        ...
        <div className="w-full md:w-[45%] bg-[#F3EAE1] p-10 md:p-16 flex flex-col justify-between font-optima text-gray-900">
          ...
          {/* Logo */}
          <div className="mb-10">
            {/* Note: Yahan 'logo-dark.png' use karna kyunki background light hai */}
            <Image 
              src="/logo.png" 
              alt="Exotica Logo" 
              width={160} 
          ...
          <div className="py-8 border-b border-gray-300">
            <h4 className="font-bold text-sm mb-3">Corporate Office</h4>
          ...
          <div className="py-8 border-b border-gray-300 grid grid-cols-2 gap-4">
            <div>
              <h4 className="font-bold text-sm mb-3">Phone Number</h4>
              <p className="text-gray-600 text-sm">123456789</p>
            </div>
            <div>
              <h4 className="font-bold text-sm mb-3">Email</h4>
        ...
        <div className="w-full md:w-[55%] bg-black flex items-center justify-center text-white py-20 md:py-0 relative">
      ...
      <div className="bg-black border-t border-white/10 text-[#666666] text-[10px] md:text-xs py-5 px-6 md:px-10 flex flex-col md:flex-row justify-between items-center gap-2 font-optima tracking-wider">
        <Link href="/disclaimer" className="hover:text-white transition-colors">
          Disclaimer
        </Link>
        <p>Copyright 2026, All Right Reserved EXOTICA.</p>
      </div>
    </footer>
  );
}

// Sub-component for Large Black Section Links
function FooterLink({ href, text }: { href: string; text: string }) {
  return (
    <Link 
      href={href} 
      // Font size kam kar diya (text-3xl md:text-4xl) aur hover effect soft kar diya
...
function SocialIcon({ icon }: { icon: string }) {
  const icons = {
    ...
    x: "M4 4l11.73 16h5L9 4zm11 14l-9-12h-3l9 12z", // X (Twitter) approximation
